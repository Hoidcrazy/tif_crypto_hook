 ...
 */
 ...
 #define FD_HASHTABLE_SIZE             1024                 // 哈希表大小（必须是2的幂）
 #define FD_HASHTABLE_MASK             (FD_HASHTABLE_SIZE - 1) // 哈希表掩码
 ...
 typedef struct fd_context_s {
     ...
     struct fd_context_s *next;      // 哈希冲突链表指针
 } fd_context_t;
 
 // 哈希表结构
 static fd_context_t *fd_hashtable[FD_HASHTABLE_SIZE];
 static pthread_mutex_t fd_hashtable_mutex = PTHREAD_MUTEX_INITIALIZER;
 
 // 哈希函数：将文件描述符映射到哈希桶
 static inline int fd_hash(int fd) {
     return fd & FD_HASHTABLE_MASK;
 }
 ...
 static void fd_context_add(int fd, const char *path) {
     ...
     int hash_index = fd_hash(fd);
     
     pthread_mutex_lock(&fd_hashtable_mutex);
     
     // 检查是否已存在（避免重复添加）
     fd_context_t *existing = fd_hashtable[hash_index];
     while (existing) {
         if (existing->fd == fd) {
             pthread_mutex_unlock(&fd_hashtable_mutex);
             DEBUG_LOG("FD上下文已存在，跳过添加: fd=%d", fd);
             return;
         }
     ...
     // 头插法插入到对应哈希桶的链表中
     node->next = fd_hashtable[hash_index];
     fd_hashtable[hash_index] = node;
     
     if (node->is_target_dwg || node->is_temp_file) {
         DEBUG_LOG("【哈希表】添加文件上下文: fd=%d, 哈希桶=%d, 路径=%s, 目标DWG=%s, 临时文件=%s, 需要加密写入=%s", 
                  fd, hash_index, path ? path : "(空)", 
 ...
 static void fd_context_remove(int fd) {
     ...
     fd_context_t **pp = &fd_hashtable[hash_index];
     while (*pp) {
         if ((*pp)->fd == fd) {
             ...
             *pp = (*pp)->next;  // 从链表中移除
             ...
             if (to_remove->is_target_dwg) {
                 DEBUG_LOG("【哈希表】移除目标DWG文件上下文: fd=%d, 哈希桶=%d, 路径=%s", 
                          fd, hash_index, to_remove->path ? to_remove->path : "(空)");
             } else if (to_remove->is_temp_file) {
                 DEBUG_LOG("【哈希表】移除临时文件上下文: fd=%d, 哈希桶=%d, 路径=%s", 
                          fd, hash_index, to_remove->path ? to_remove->path : "(空)");
             }
 ...
 static int fd_context_get_info(int fd, fd_context_t *out) {
     ...
     fd_context_t *current = fd_hashtable[hash_index];
     while (current) {
         if (current->fd == fd) {
             // 找到匹配的FD，复制信息
             out->fd = current->fd;
             ...
             out->saw_plain_writes = current->saw_plain_writes;
 ...
 static void mark_path_encrypt_on_write(const char *path, int should_encrypt) {
     ...
     // 遍历所有哈希桶
     for (int i = 0; i < FD_HASHTABLE_SIZE; i++) {
         fd_context_t *current = fd_hashtable[i];
         while (current) {
             if (current->path && strcmp(current->path, path) == 0) {
                 current->should_encrypt_on_write = should_encrypt;
                 DEBUG_LOG("【哈希表】标记路径写入加密: %s, 哈希桶=%d, 加密=%s", 
                          path, i, should_encrypt ? "是" : "否");
             }
             current = current->next;
         }
     }
 ...
 // 【优化】标记FD见过明文写入（哈希表版本）
 static void mark_fd_saw_plain_writes(int fd) {
     if (fd < 0) return;
 ...
 static void final_seal_if_needed(int fd) {
     ...
     if (!ctx.saw_plain_writes) {
         DEBUG_LOG("【哈希表】fd=%d 未检测到明文写入，跳过收尾检查", fd);
         if (ctx.path) free(ctx.path);
         return;
     }
     
     DEBUG_LOG("【哈希表】开始收尾检查: fd=%d, 路径=%s", fd, ctx.path ? ctx.path : "未知");
 ...
 int close(int fd) {
     ...
     if (fd_context_get_info(fd, &ctx)) {
         if (ctx.is_target_dwg) {
             DEBUG_LOG("【哈希表】关闭目标DWG文件，执行收尾检查: fd=%d, 路径=%s", 
 ...
 ssize_t read(int fd, void *buf, size_t count) {
     ...
     memset(&ctx, 0, sizeof(ctx));
     if (fd_context_get_info(fd, &ctx) && ctx.is_target_dwg) {
         DEBUG_LOG("=====【读取Hook】【哈希表】fd=%d, 文件=%s, 字节数=%zd", 
 ...
 size_t fread(void *ptr, size_t size, size_t nmemb, FILE *stream) {
     ...
     if (fd >= 0) {
         ...
         if (fd_context_get_info(fd, &ctx) && ctx.is_target_dwg) {
             ...
             DEBUG_LOG("=====【fread Hook】【哈希表】fd=%d, 文件=%s, 字节数=%zu", 
 ...
 ssize_t write(int fd, const void *buf, size_t count) {
     ...
     // 【优化】标记见过明文写入（使用哈希表版本）
     mark_fd_saw_plain_writes(fd);
     
     DEBUG_LOG("=====【写入Hook】【哈希表】fd=%d, 大小=%zu字节 (方案2在线加密)", fd, count);
 ...
 ssize_t pwrite64(int fd, const void *buf, size_t count, off_t offset) {
     ...
     DEBUG_LOG("=====【pwrite64 Hook】【哈希表】fd=%d, 偏移=%ld, 大小=%zu (方案2在线加密)", 
 ...
 ssize_t pwrite(int fd, const void *buf, size_t count, off_t offset) {
     ...
     DEBUG_LOG("=====【pwrite Hook】【哈希表】fd=%d, 偏移=%ld, 大小=%zu (方案2在线加密)", 
              fd, (long)offset, count);
 ...
 size_t fwrite(const void *ptr, size_t size, size_t nmemb, FILE *stream) {
     ...
     DEBUG_LOG("=====【fwrite Hook】【哈希表】fd=%d, 字节数=%zu (方案2在线加密)", fd, total_bytes);
 ...
 void *mmap(void *addr, size_t length, int prot, int flags, int fd, off_t offset) {
     ...
     if (fd >= 0) {
         if (fd_context_get_info(fd, &ctx)) {
             ...
             if (path) {
                 ...
                 file_path = path;
                 DEBUG_LOG("【哈希表】从fd推断文件类型: fd=%d, 路径=%s, 是否目标DWG=%s", 
     ...
     if (is_target && (flags & MAP_SHARED)) {
         ...
         DEBUG_LOG("【方案1】【哈希表】检测到目标DWG的MAP_SHARED映射，转换为MAP_PRIVATE私有副本");
         ...
         if (is_target) {
             ...
             DEBUG_LOG("【哈希表】对目标DWG非共享映射进行解密: 地址=%p, 长度=%zu", result, length);
             if (decrypt_memory_region(result, length, prot) != 0) {
 ...
 int munmap(void *addr, size_t length) {
     ...
     if (region && region->is_target_dwg && region->should_preserve) {
         DEBUG_LOG("【方案1】【哈希表】DWG私有副本解除映射前先备份: 地址=%p, 长度=%zu, fd=%d", 
 ...
 int fsync(int fd) {
     ...
     memset(&ctx, 0, sizeof(ctx));
     if (fd_context_get_info(fd, &ctx) && ctx.is_target_dwg) {
         DEBUG_LOG("【哈希表】目标DWG文件同步: fd=%d, 路径=%s", fd, ctx.path ? ctx.path : "(未知)");
 ...
 int fdatasync(int fd) {
     ...
     memset(&ctx, 0, sizeof(ctx));
     if (fd_context_get_info(fd, &ctx) && ctx.is_target_dwg) {
         DEBUG_LOG("【哈希表】目标DWG文件数据同步: fd=%d, 路径=%s", fd, ctx.path ? ctx.path : "(未知)");
 ...
 int rename(const char *oldpath, const char *newpath) {
     ...
     if (oldpath && newpath) {
         if (is_target_dwg_file(oldpath) || is_target_dwg_file(newpath)) {
             DEBUG_LOG("【哈希表】DWG文件重命名操作: '%s' -> '%s'", oldpath, newpath);
         }
         // 【方案2】临时文件重命名为目标DWG文件
         if (is_temporary_file(oldpath) && is_target_dwg_file(newpath)) {
             DEBUG_LOG("【方案2】【哈希表】检测到临时文件保存为DWG: '%s' -> '%s'", oldpath, newpath);
         }
     }
     ...
     if (oldpath && newpath) {
         if (is_target_dwg_file(newpath)) {
             DEBUG_LOG("【哈希表】检测到保存操作：临时文件 -> DWG文件");
             ...
             if (ENABLE_FINAL_FILE_ENCRYPTION) {
                 ...
                 for (int i = 0; i < MAX_MMAP_REGIONS; i++) {
                     ...
                     if (region->addr && region->is_target_dwg && region->is_private_copy) {
                         ...
                         if (region->file_path) free(region->file_path);
                         region->file_path = strdup(newpath);
                         DEBUG_LOG("【方案1】【哈希表】标记私有副本延迟处理: addr=%p, 新路径=%s", region->addr, newpath);
                     }
                 }
                 pthread_mutex_unlock(&mmap_table_mutex);
             }
 ...
 int renameat(int olddirfd, const char *oldpath, int newdirfd, const char *newpath) {
     ...
     if (oldpath && newpath) {
         if (is_target_dwg_file(oldpath) || is_target_dwg_file(newpath)) {
             DEBUG_LOG("【哈希表】DWG文件重命名操作（at版本）: '%s' -> '%s'", oldpath, newpath);
         }
         // 【方案2】临时文件重命名为目标DWG文件
         if (is_temporary_file(oldpath) && is_target_dwg_file(newpath)) {
             DEBUG_LOG("【方案2】【哈希表】检测到临时文件保存为DWG（at版本）: '%s' -> '%s'", oldpath, newpath);
         }
     }
     ...
     if (oldpath && newpath) {
         if (is_target_dwg_file(newpath)) {
             DEBUG_LOG("【哈希表】检测到保存操作（at版本）：临时文件 -> DWG文件");
             ...
             if (ENABLE_FINAL_FILE_ENCRYPTION) {
                 ...
                 for (int i = 0; i < MAX_MMAP_REGIONS; i++) {
                     ...
                     if (region->addr && region->is_target_dwg && region->is_private_copy) {
                         ...
                         if (region->file_path) free(region->file_path);
                         region->file_path = strdup(newpath);
                         DEBUG_LOG("【方案1】【哈希表】标记私有副本延迟处理（at版本）: addr=%p, 新路径=%s", region->addr, newpath);
                     }
                 }
                 pthread_mutex_unlock(&mmap_table_mutex);
             }
 ...
 // ==================== 哈希表统计和调试函数 ====================
 static void print_fd_hashtable_stats(void) {
     if (!is_debug_enabled()) return;
     ...
     for (int i = 0; i < FD_HASHTABLE_SIZE; i++) {
         ...
         if (current) {
             ...
             while (current) {
                 total_entries++;
                 chain_length++;
                 if (current->is_target_dwg) target_dwg_count++;
                 if (current->is_temp_file) temp_file_count++;
                 current = current->next;
             }
     ...
     DEBUG_LOG("【哈希表统计】总条目=%d, 使用桶数=%d/%d, 最长链=%d, 目标DWG=%d, 临时文件=%d",
              total_entries, used_buckets, FD_HASHTABLE_SIZE, max_chain_length, 
              target_dwg_count, temp_file_count);
 ...
 static void dwg_hook_initialize(void) {
     ...
     if (is_debug_enabled()) {
         ...
         if (log_file) {
             fprintf(log_file, "DWG加密Hook库初始化开始（哈希表版本）\n");
             fclose(log_file);
         }
     }
     
     DEBUG_LOG("=============================================");
     DEBUG_LOG("DWG文件透明加解密Hook库 - 哈希表优化版本");
     DEBUG_LOG("【哈希表优化】FD管理从O(n)线性查找优化为O(1)哈希查找");
     DEBUG_LOG("【哈希表大小】%d个桶，使用FD值进行哈希分布", FD_HASHTABLE_SIZE);
 ...
 static void dwg_hook_cleanup(void) {
     DEBUG_LOG("【哈希表】DWG Hook库清理开始...");
     
     // 打印最终统计信息
     print_fd_hashtable_stats();
     ...
     for (int i = 0; i < FD_HASHTABLE_SIZE; i++) {
         fd_context_t *current = fd_hashtable[i];
         while (current) {
             fd_context_t *next = current->next;
     ...
     DEBUG_LOG("【哈希表】DWG Hook库清理完成，共处理%d个私有副本", cleanup_count);
     DEBUG_LOG("【哈希表】所有FD上下文已清理");
     DEBUG_LOG("=============================================");
 }