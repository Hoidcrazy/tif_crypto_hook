...
// 功能: 通过 LD_PRELOAD 拦截 pread64 、 read 、mmap 、mmap64 系统调用，对特定加密的 .dwg 文件进行透明解密（0xFF 异或）
// 适用场景: 中望CAD (ZWCAD) 读取被加密的 DWG 文件（文件名包含 "changed_" 且以 ".dwg" 结尾）

// 编译命令:
//   gcc -shared -fPIC -o libdwg_hook.so dwg_hook.c -ldl -lpthread
//
// LD_PRELOAD=/home/chane/tif_crypto_hook/libdwg_hook.so /opt/apps/zwcad2025/ZWCADRUN.sh 2> ~/hook_debug.log
// less ~/hook_debug.log
//
// 使用方法（挂载到中望CAD启动脚本）:
...
//   重定向输出查看日志: LD_PRELOAD=... ./ZWCAD ... 2> ~/hook_debug.log

#define _GNU_SOURCE  // 启用 GNU 扩展功能（如 strcasecmp, va_start 等）
#include <dlfcn.h>   // dlsym, RTLD_NEXT - 用于获取原始函数指针
#include <stdio.h>   // fprintf, stderr, snprintf - 错误和调试输出
#include <string.h>  // strcmp, strcasecmp, strstr, strlen, strdup - 字符串操作
#include <fcntl.h>   // openat, O_CREAT, O_RDONLY 等标志
#include <unistd.h>  // read, close, readlink - 系统调用
#include <stdlib.h>  // getenv, free, malloc - 环境变量和内存管理
#include <sys/types.h>
#include <sys/syscall.h>
#include <sys/stat.h> // fstat 等（虽然未直接使用，但常与文件操作相关）
#include <sys/mman.h>
#include <stdarg.h>   // va_list, va_start, va_end - 处理可变参数
#include <limits.h>   // PATH_MAX - 路径最大长度
#include <pthread.h>  // pthread_mutex_t, pthread_mutex_lock - 线程同步
#include <errno.h>
